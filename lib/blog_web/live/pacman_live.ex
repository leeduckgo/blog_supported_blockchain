defmodule BlogWeb.PacmanLive do
  use Phoenix.LiveView
  alias BlogWeb.Router.Helpers, as: Routers
    ...
    <div class="game-container">
      <div phx-keydown="keydown"
          ...
          style="transform: rotate(<%= @rotation %>deg);
                 ...
                 height: <%= @width %>px;
      ">
        <img style="width: <%= @width %>px; height: <%= @width %>px;" src="<%= Routers.static_path(BlogWeb.Endpoint, "/images/doge.png") %>" alt="PacMan Go!"/>
  ...
  def go(socket, direction) do
    facing_ele_coord = generate_facing_ele_coord(socket, direction)
    do_go(socket, facing_ele_coord)
  end

  defp generate_facing_ele_coord(socket, :left) do
    [socket.assigns.row, socket.assigns.col - 1]
  end

  defp generate_facing_ele_coord(socket, :right) do
    [socket.assigns.row, socket.assigns.col + 1]
  end

  defp generate_facing_ele_coord(socket, :up) do
    [socket.assigns.row - 1, socket.assigns.col]
  end

  defp generate_facing_ele_coord(socket, :down) do
    [socket.assigns.row + 1, socket.assigns.col]
  end

  defp do_go(socket, [row, col]) do
    board = @board
    facing_ele = board |> Enum.fetch!(row) |> Enum.fetch!(col)
  ...
  defp advance(socket) do
    %{width: width, heading: _heading, blocks: blocks} = socket.assigns
