defmodule BlogWeb.YcyRealEstateController do
  ...
  def buy(conn, %{"api" => api_key, "name" => name, "buyer" => buyer_puid, "group_id" => group_puid, "amount" => amount}) do
  ...
  def update(conn, %{"api" => api_key, "user" => user_puid, "group_id" => group_puid, "name" => name, "signature" => signature}) do
    if Auth.auth?(api_key) do
      real_estate =
        name
        |> YcyRealEstate.get_real_estate()
